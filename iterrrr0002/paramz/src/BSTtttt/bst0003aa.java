...
public class bst0003aa <T extends Comparable<T>> implements Iterable<T>
{ int dodaj=0;
   public static void main(String[] args)
   {
      Integer[] a = {1,50,2,7,4};
      bst0003aa<Integer> bst = new bst0003aa<Integer>();
      for(Integer n : a) bst.insert(n);

      bst.preOrderTraversal();
      System.out.println();   //1 5 2 4 7 
      ...
      //TEN DRUGI KONSTRUKTOR tutaj   public bst0003aa(Comparator<T> comp)   {
      //root = null;
      //comparator = comp;   }
 //class MyComp1 implements Comparator<Integer>
 //     {   public int compare(Integer x, Integer y) {  return y-x; }}
   
      bst = new bst0003aa<Integer>(new MyComp1());
      for(Integer n : a) bst.insert(n);

      bst.preOrderTraversal();     // 1 50 2 7 4 
      System.out.println();
      bst.inOrderTraversal();        //50 7 4 2 1 
      System.out.println();      

      for(Integer n : bst) System.out.print(n);   //150274
      System.out.println();

      System.out.println(bst);   // 150274
...
//      System.out.println("width = " + bst.width());
   ...
   public bst0003aa(Comparator<T> comp)   {
      root = null;
      comparator = comp;   }
...
/**********************   preOrderTraversal         TRAVERSAL  **************************************************/
   ...
   private boolean search(Node<T> p, T toSearch)   {
      ...
      else          return search(p.right, toSearch);
   }
   /********************************************     DELETE******************************************************/
   ...
   private Node<T> delete(Node<T> p, T toDelete)   {
      ...
      else  if (compare(toDelete, p.data) < 0)    p.left = delete (p.left, toDelete);
      else  if (compare(toDelete, p.data)  > 0)   p.right = delete (p.right, toDelete);
      else      {
...
///*************************************************   CLONE  **************************************************/
//
//   public bst0003<T> clone2()   {
//      bst0003<T> twin = null;
//
//      if(comparator == null)        twin = new bst0003<T>();
//      else       twin = new bst0003<T>(comparator);
...
//      else         return new Node<T>(p.data, cloneHelper(p.left), cloneHelper(p.right));
//   }
/*************************************************   MISC  **************************************************/
   ...
   private int height(Node<T> p)   {
      if(p == null) return -1;
      else      return 1 + Math.max( height(p.left), height(p.right));
   }
  ...
  //This method restores a BST given preorder and inorder traversals
   public void restore(T[] pre, T[] in) { root = restore(pre, 0, pre.length-1, in, 0, in.length-1);   }
...
/**************************************************** TREE ITERATOR******************************************************/
   ...
   private class MyIterator implements Iterator<T>
   {
      ...
      public MyIterator()      {         if(root != null) stk.push(root);      }
...
class MyComp1 implements Comparator<Integer>{   public int compare(Integer x, Integer y) {  return y-x; }}