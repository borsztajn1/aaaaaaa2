...
public class bst0009<E extends Comparable<E>> extends AbstractTree22<E> {
  ...
  public bst0009(E[] objects) {    for (int i = 0; i < objects.length; i++)       insert(objects[i]);  }

  @Override /** Returns true if the element is in the tree */
  public boolean search(E e) {     TreeNode<E> current = root; // Start from the root
  ...
  public boolean delete(E e) {
    ...
    TreeNode<E> current = root;
    while (current != null) {
      if (e.compareTo(current.element) < 0) {        parent = current;
        current = current.left;      }
  ...
  private class InorderIterator implements java.util.Iterator<E> {
    ...
    public boolean hasNext() {      if (current < list.size())        return true;
      return false;    }
