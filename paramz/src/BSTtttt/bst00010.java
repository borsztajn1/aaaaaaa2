	...
	public void addNode(int key, String name) {
		...
		if (root == null) {			root = newNode; } else {
			...
			while (true) {				// root is the top parent so we start there
				...
				if (key < focusNode.key) {					// Switch focus to the left child
					focusNode = focusNode.leftChild;
					// If the left child has no children
					if (focusNode == null) {		// then place the new node on the left of it
...
//		System.out.println(theTree.findNode(75));
	public Node findNode(int key) {		// Start at the top of the tree
		Node focusNode = root;
		System.out.println(root+"yyyyyyy"); //boss

		// While we haven't found the Node
		// keep looking

		while (focusNode.key != key) {			// If we should search to the left
			if (key < focusNode.key) {				// Shift the focus Node to the left child
				focusNode = focusNode.leftChild;			} else {
...
class Node {
	...
	public String toString() {return name + " has the key " + key;
