...
// Prints all the values integers.
{
 ...
 {
   value = values[index];
   if (((index + 1) % 10) == 0)    System.out.println(fmt.format(value));
...
// otherwise return false
{
 ...
 while (first <= last)
 {
   ...
   if (target == values[midpoint])        return true;
...
// otherwise return false
{
 ...
 else   if (target == values[midpoint])      return true;
   else    if (target > values[midpoint])  // target too high
